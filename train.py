...
import copy
from utils.NNModel import MLP_H
import gc
import os
import argparse
...
parser.add_argument('-MNGD', '--Modified_NGD', default=False, action='store_true')
...
train_loader,valid_loader,test_loader = read_dataset(batch_size=batch_size, data_path=f'./data/htru2/perturbed_with_noise/2pm{delta}/')
...
def train(model, mode='SGD', lr_decay=False): 
    ...
    for epoch in tqdm(range(1, n_epochs+1)):
        ...
        if test_acc > max_test_acc and epoch>=190:
            if os.path.isdir(f'checkpoint/htru2/perturbed_with_noise/2pm{delta}/{mode}/') == False:
                os.makedirs(f'checkpoint/htru2/perturbed_with_noise/2pm{delta}/{mode}/')
            max_test_acc = test_acc
            # torch.save(model.state_dict(), f'checkpoint/Modified_NGD/mlp_epoch{epoch}.pt')
            torch.save(model.state_dict(), f'checkpoint/htru2/perturbed_with_noise/2pm{delta}/{mode}/mlp_epoch{epoch}_seed{seed}.pt')
        # 每遍历一遍数据集，测试一下准确率
        torch.cuda.empty_cache()
    ...
    # NO cut
    if os.path.isdir(f'results_H/perturbed_with_noise/2pm{delta}/{mode}') == False:
        os.makedirs(f'results_H/perturbed_with_noise/2pm{delta}/{mode}')
    np.save(f'results_H/perturbed_with_noise/2pm{delta}/{mode}/train_loss_seed{seed}.npy', np.array(Train_loss))
    np.save(f'results_H/perturbed_with_noise/2pm{delta}/{mode}/test_acc_seed{seed}.npy', np.array(Test_acc))
    np.save(f'results_H/perturbed_with_noise/2pm{delta}/{mode}/validation_acc_seed{seed}.npy', np.array(Valid_acc))
    if mode != 'SGD':
        # np.save(f'results_H/{perturb}/{sigma}/2pm{delta}/{mode}/theoretical_loss{seed}.npy', np.array(theoretical_loss_list))
        np.save(f'results_H/perturbed_with_noise/2pm{delta}/{mode}/preserved_eigens{seed}.npy', np.array(preserved_eigens_list))

    # Cut small
    # if os.path.isdir(f'results_H/{perturb}/{sigma}/{mode}_cut_small') == False:
    #     os.makedirs(f'results_H/{perturb}/{sigma}/{mode}_cut_small')
    ...
    #     os.makedirs(f'results_H/{perturb}/{sigma}/{mode}_cut_large')
