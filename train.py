...
def train(model, mode='SGD', lr_decay=False): 
    ...
    if mode != 'SGD':
        ...
        np.save(f'results/{perturb}/{sigma}/{mode}/preserved_eigens{seed}.npy', np.array(preserved_eigens_list))

    # Cut small
    # os.mkdir(f'results/{perturb}/{sigma}/{mode}_cut_small/')
    # np.save(f'results/{perturb}/{sigma}/{mode}_cut_small/train_loss_seed{seed}.npy', np.array(Train_loss))
    # np.save(f'results/{perturb}/{sigma}/{mode}_cut_small/test_loss_seed{seed}.npy', np.array(Test_loss))
    # np.save(f'results/{perturb}/{sigma}/{mode}_cut_small/validation_loss_seed{seed}.npy', np.array(Valid_loss))
    # if mode != 'SGD':
    #     # np.save(f'results/{perturb}/{sigma}/{mode}/theoretical_loss{seed}.npy', np.array(theoretical_loss_list))
    #     np.save(f'results/{perturb}/{sigma}/{mode}_cut_small/preserved_eigens{seed}.npy', np.array(preserved_eigens_list))

    # # Cut large
    # os.mkdir(f'results/{perturb}/{sigma}/{mode}_cut_large/')
    # np.save(f'results/{perturb}/{sigma}/{mode}_cut_large/train_loss_seed{seed}.npy', np.array(Train_loss))
    # np.save(f'results/{perturb}/{sigma}/{mode}_cut_large/test_loss_seed{seed}.npy', np.array(Test_loss))
    # np.save(f'results/{perturb}/{sigma}/{mode}_cut_large/validation_loss_seed{seed}.npy', np.array(Valid_loss))
    # if mode != 'SGD':
    #     # np.save(f'results/{perturb}/{sigma}/{mode}/theoretical_loss{seed}.npy', np.array(theoretical_loss_list))
